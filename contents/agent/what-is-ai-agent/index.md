...
summary: 'AI 에이전트를 구성하는 세 가지 본질적 요소인 모델, 도구, 루프. Anthropic과 OpenAI의 공식 가이드를 기반으로, 에이전트가 실제로 어떻게 설계되는지 그 구조를 처음부터 살펴봅니다.'
...
<svg viewBox="0 0 480 300" style="width: 100%; height: auto; max-width: 480px;" xmlns="http://www.w3.org/2000/svg" font-family="Pretendard, -apple-system, sans-serif" role="img" aria-label="Augmented LLM 구조도. 도구, 검색, 메모리 세 요소가 중앙의 LLM 코어에 연결되어 하나의 증강된 LLM을 이룬다.">
  ...
  <text x="240" y="52" font-size="22" font-weight="700" text-anchor="middle" fill="var(--text, #1c1917)">Augmented LLM</text>
  <!-- 증강 요소 세 가지 -->
  <rect x="38" y="76" width="124" height="60" rx="10" fill="var(--bg, #fafaf8)" stroke="var(--border, #e7e5e4)" stroke-width="1.5" />
  <text x="100" y="113" font-size="20" text-anchor="middle" fill="var(--text, #1c1917)">Tools</text>
  <rect x="178" y="76" width="124" height="60" rx="10" fill="var(--bg, #fafaf8)" stroke="var(--border, #e7e5e4)" stroke-width="1.5" />
  <text x="240" y="113" font-size="20" text-anchor="middle" fill="var(--text, #1c1917)">Retrieval</text>
...
구현 방식은 다르지만 본질은 동일합니다. 모델이 도구 호출을 요청하면 실행하고, 결과를 모델에 돌려주고, 모델이 더 이상 도구 호출을 하지 않을 때까지 반복합니다. 이 루프 자체는 놀라울 정도로 단순합니다. 복잡한 것은 루프 **주변**의 인프라(컨텍스트 관리, 퍼미션, 에러 복구)이며, 이 시리즈의 뒤쪽 글들에서 자세히 다룹니다.

---

## 루프가 왜 그렇게 중요한가

루프의 힘을 보여주는 인상적인 수치가 있습니다. Andrew Ng이 에이전틱 디자인 패턴의 효과를 설명하며 인용한 HumanEval 코딩 벤치마크 결과입니다.
