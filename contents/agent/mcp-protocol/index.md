...
Claude Code에 GitHub, Slack, Sentry, Grafana, Splunk 다섯 개의 외부 서비스를 연결하면 58개의 도구가 등장합니다. 이 도구들의 스키마 정의만으로 약 55,000 토큰을 소비합니다. 대화가 시작되기도 전에 200K 토큰 윈도우의 4분의 1 이상이 도구 설명으로 채워지는 것입니다. 서비스를 추가할수록 상황은 악화됩니다. 도구가 많아질수록 에이전트가 오히려 느려지는 아이러니입니다.
...
MCP(Model Context Protocol)는 이 두 문제를 동시에 해결하려는 표준 프로토콜입니다. 이 글에서는 MCP의 아키텍처, 도구 폭발을 줄이는 최적화 전략, 그리고 에이전트 루프와의 통합을 다룹니다.
...
**Streamable HTTP**: 원격 서버 통신입니다. HTTP POST로 요청을 보내고, Server-Sent Events(SSE)로 응답을 스트리밍합니다. 클라우드에 배포된 MCP 서버나 공유 서비스에 적합합니다. 인증, 라우팅, 로드밸런싱 같은 HTTP 인프라를 그대로 활용합니다.
...
MCP 도구는 다릅니다. 서드파티가 작성한 코드가 실행됩니다. `github-mcp-server`의 `create_pull_request` 도구가 실제로 무엇을 하는지, 에이전트 개발팀은 코드를 읽지 않는 한 확신할 수 없습니다. MCP 스펙은 이 문제를 명확한 원칙으로 짚습니다.
...
도구의 설명(description)과 어노테이션(annotations)조차 신뢰할 수 없다는 뜻입니다. `readOnlyHint: true`라고 선언된 도구가 실제로는 데이터를 수정할 수도 있습니다. 힌트(hint)일 뿐, 보장(guarantee)이 아닙니다.

### Tool Annotations

MCP 스펙은 도구의 동작을 설명하는 어노테이션을 정의한다.
...
MCP의 가장 중요한 보안 설계 원칙 중 하나는 **서버 간 격리**다. MCP 스펙은 이렇게 명시합니다.
...
모델이 `tool_search`를 호출하면 deferred 도구를 탐색하여 active로 승격하고, 다음 턴부터 해당 도구를 호출합니다. 모델이 MCP 도구를 호출하면, 해당 도구가 등록된 Server의 Client를 찾아서 `call_tool()`을 실행합니다.
...
| 현재 채택 | 프로덕션 수준 (Claude Code, Cursor 등) | v1.0 스펙 공개, 채택 확산 중 |

A2A의 핵심 개념은 **Agent Card**와 **Task lifecycle**입니다. Agent Card는 에이전트가 자신의 능력을 JSON으로 선언하는 방식으로, MCP Server의 capability 교환에 대응하는 개념입니다. Task는 에이전트 간에 위임되는 작업 단위로, submitted, working, input-required, completed 같은 상태 전이를 거칩니다.

A2A는 v1.0 안정 스펙이 공개되었고 150개 이상의 조직이 참여하고 있지만, MCP에 비하면 프로덕션 채택은 아직 초기입니다. MCP가 이미 주요 에이전트에 광범위하게 통합된 것과 달리, A2A의 실제 프로덕션 배포는 Azure AI Foundry, Amazon Bedrock AgentCore 등 클라우드 플랫폼 중심으로 시작되는 단계입니다.
...
MCP가 "누구나 서버를 만들 수 있다"는 것은 장점이자 단점입니다. npm 생태계에서 패키지 품질이 천차만별인 것처럼, MCP 서버의 도구 설계 품질도 크게 다릅니다. [세 번째 글](/agent/agent-tool-use/)에서 다룬 ACI 원칙(명확한 네이밍, 최소한의 파라미터, 의미 있는 에러 메시지)을 지키는 서버가 있는 반면, 모호한 설명과 과도한 파라미터로 모델을 혼란스럽게 만드는 서버도 있습니다. 도구 설계가 곧 모델의 성능을 결정하므로, 서버 품질 문제는 에이전트 전체의 성능에 직접 영향을 미친다.

### 상태 관리

현재 MCP에서 서버 간 상태 공유는 지원되지 않습니다. GitHub Server에서 생성한 PR의 URL을 Slack Server에 전달하려면, Host가 중간에서 값을 추출하여 다음 도구 호출에 직접 전달해야 합니다. 서버 간 직접 통신 채널이 없으므로, 복잡한 워크플로우에서는 Host의 오케스트레이션 부담이 커지게 된다.
...
서드파티 MCP 서버를 사용한다는 것은 외부 코드를 실행하는 셈입니다. npm의 `npm audit`에 해당하는 MCP 서버 보안 감사 도구나 표준은 아직 없습니다. 서버의 소스 코드를 직접 검토하거나, 신뢰할 수 있는 공급자의 서버만 사용하는 것이 현재로서는 유일한 방어 수단입니다.
