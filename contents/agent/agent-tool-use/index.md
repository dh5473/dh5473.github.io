...
summary: 'Anthropic이 제안하는 ACI(Agent-Computer Interface) 설계 원칙과 도구 설계 5원칙. 네이밍, 스키마 설계, 에러 시그널, 멱등성까지 Claude Code의 실제 도구를 해부하며 프로덕션 수준의 도구 설계를 이해한다.'
...
이전 글에서 다섯 가지 워크플로우 패턴을 살펴봤는데, 어떤 패턴을 쓰든 결국 도구 설계가 잘못되면 성능이 나오지 않습니다. 이 글에서는 Anthropic이 제안하는 **ACI(Agent-Computer Interface)** 설계 원칙과, 프로덕션 에이전트의 실제 도구 스키마를 분석합니다.
...
Anthropic은 ACI의 핵심 원칙으로 세 가지를 제시합니다.

- **단순성(Simplicity)**: 도구의 수와 복잡도를 최소화
- **투명성(Transparency)**: 모델이 자신의 행동을 명시적으로 계획할 수 있게
- **신중한 설계(Careful crafting)**: 문서화와 테스트를 통한 반복 개선

핵심 설계 휴리스틱은 한 문장으로 요약됩니다. **"Put yourself in the model's shoes."** 모델의 입장에서 생각하라. 이 도구 이름만 보고 어떤 기능인지 알 수 있는가? 이 설명만 읽고 언제 써야 하는지 판단할 수 있는가? 이 에러 메시지만 보고 다음에 무엇을 해야 하는지 결정할 수 있는가?
...
세 번의 반복이 필요했습니다. HCI에서 UI를 사용자 테스트로 개선하듯, ACI에서 도구를 모델 테스트로 개선하는 것입니다. Claude Code 블로그에서는 이를 이렇게 표현합니다. "Each new tool represents another decision point for the model." 도구 하나를 추가하는 것은 모델의 의사결정 공간을 넓히는 것이기 때문에, 높은 기준을 적용해야 합니다.
...
Anthropic은 "Writing Effective Tools for AI Agents"에서 다섯 가지 설계 원칙을 제시합니다. 이것은 추상적인 가이드라인이 아닙니다. Claude Code의 도구를 수백만 사용자 환경에서 반복 개선하면서 추출한 실전 원칙입니다.
...
`search_contacts` 하나가 네 도구의 기능을 모두 포함합니다. 쿼리 없이 호출하면 전체 목록, `tag`를 주면 필터링, `sort_by: "recent"`를 주면 최근 순 정렬. 모델의 선택지가 4개에서 1개로 줄어들면서, 잘못된 도구를 고를 확률이 사라집니다.

Claude Code는 이 원칙을 철저히 따릅니다. 내부적으로 최대 54개의 도구를 등록할 수 있지만, 실제로 사용하는 핵심 도구는 약 20개입니다. 나머지는 기능 플래그와 사용자 유형에 따라 조건부로 활성화됩니다.

### 원칙 2. 네이밍과 네임스페이싱

도구 이름은 모델이 가장 먼저 읽는 정보입니다. 이름만으로 어떤 도구인지 판단이 안 되면, 모델은 설명을 읽어야 하고, 그만큼 추론에 토큰을 소비합니다.
...
각 이름이 하나의 행동을 즉시 전달합니다. `Read`와 `Grep`은 둘 다 파일을 읽지만, 이름만으로 차이가 명확합니다. "특정 파일의 내용을 읽는 것"과 "패턴으로 검색하는 것". 이 명확한 구분이 글 서두에서 본 Claude Code의 도구 선택 행동을 만들어냅니다.
...
Anthropic은 SWE-bench Verified에서 도구 설명을 다듬는 것만으로 의미 있는 성능 향상을 얻었다고 보고합니다. 코드를 한 줄도 바꾸지 않고, 설명만 개선했을 때의 결과입니다. 또한 도구에 1~5개의 사용 예시를 추가하면 복잡한 파라미터 처리 정확도가 72%에서 90%로 올라갔습니다.
...
아래 설명에는 "무엇을 하는가"(문자열 치환), "전제 조건"(먼저 Read), "실패 조건"(old_string 고유하지 않으면), "행동 지침"(새 파일 생성보다 기존 파일 편집 선호)이 모두 포함되어 있습니다. 이것은 "프롬프트 엔지니어링"과 본질적으로 같은 작업입니다.
...
도구가 반환하는 값은 모델의 컨텍스트 윈도우에 들어갑니다. 반환값이 클수록 모델이 추론에 쓸 수 있는 공간이 줄어듭니다.

Anthropic은 반환값 설계에서 두 가지를 강조합니다. 첫째, **의미 있는 컨텍스트를 반환하라**. UUID 같은 내부 식별자 대신 사람(그리고 모델)이 이해할 수 있는 이름을 포함해야 합니다.
...
둘째, **토큰 효율을 관리하라**. Anthropic은 `ResponseFormat` 개념을 소개합니다. 같은 데이터를 DETAILED 포맷(206 토큰)과 CONCISE 포맷(72 토큰)으로 반환하는 것이 66%의 토큰 절감을 가져옵니다. 상황에 따라 필요한 수준의 상세도를 선택하는 것입니다.
...
`truncated: True`와 `suggestion` 필드가 핵심입니다. 단순히 자르는 것이 아니라, 모델에게 "잘렸으니 더 구체적인 쿼리를 써라"고 안내합니다.

### 원칙 5. 에러 시그널과 Poka-yoke

도구 실행이 실패했을 때 모델이 받는 에러 메시지는, 다음 행동을 결정하는 유일한 정보입니다. "Error occurred"만 반환하면 모델은 같은 실수를 반복할 수밖에 없습니다.

Anthropic은 **실행 가능한(actionable) 에러 메시지**를 강조합니다. 무엇이 잘못되었는지, 어떻게 고쳐야 하는지를 에러 자체에 포함시켜야 합니다.
...
Anthropic이 SWE-bench에서 발견한 사례가 대표적입니다. 파일 편집 도구가 상대 경로를 허용했을 때, 모델이 작업 디렉터리를 이동한 후 잘못된 경로를 참조하는 버그가 반복적으로 발생했습니다. 해결책은 단순했습니다. **절대 경로만 허용하도록 변경**한 것입니다. Anthropic에 따르면 이 변경 후 "모델이 이 방법을 완벽하게 사용했다"고 합니다.
...
에러 메시지에 올바른 절대 경로를 포함시키면, 모델은 즉시 수정된 경로로 재시도할 수 있습니다. 실수를 방지하고, 실수가 발생해도 복구 경로를 제공하는 이중 안전장치입니다.
...
Claude Code의 도구 설계에서 이 구분이 선명합니다. `Write` 도구는 파일 전체를 덮어씁니다. 같은 내용으로 두 번 호출하면 결과는 동일합니다. `Edit` 도구는 diff 기반으로 동작합니다. 매칭되는 문자열을 찾아 교체하는데, 이미 교체된 상태에서 다시 호출하면 원본 문자열을 찾지 못해 "no changes made"를 반환합니다. 파일이 중복 수정되는 것을 구조적으로 방지하는 것입니다.
...
`set_config`는 몇 번을 호출해도 마지막 값이 유지됩니다. `increment_counter`는 호출할 때마다 값이 올라갑니다. 에이전트 환경에서 비멱등 도구는 확인 게이트(human-in-the-loop)를 필수로 두거나, 멱등한 대안(`set_counter(name, value)`)으로 교체하는 것이 안전합니다.

---

## 프로덕션 비교: 도구 설계 현황

두 프로덕션 에이전트의 도구 설계 접근 방식을 비교해 보겠습니다.
...
둘째, 안전성 확보의 철학이 다릅니다. Claude Code는 **도구 수준에서 분류**(이전 글에서 다룬 `partitionToolCalls()`)합니다. 읽기 전용 도구는 병렬 실행, 쓰기 도구는 순차 실행으로 안전성을 확보합니다. Codex는 **환경 수준에서 격리**합니다. 모든 도구가 샌드박스 컨테이너 안에서 실행되므로, 개별 도구의 안전성을 분류할 필요가 줄어듭니다. 같은 목표를 서로 다른 레이어에서 해결하는 것입니다.
...
에이전트가 MCP를 통해 외부 서비스를 연결하기 시작하면, 도구 수가 급격히 늘어납니다. GitHub 35개, Slack 11개, Sentry 5개 같은 MCP 서버를 연결하면 도구 정의만으로 55,000 토큰 이상을 소비할 수 있습니다. 대화가 시작되기도 전에 컨텍스트 절반이 도구 설명으로 채워지는 것입니다. Anthropic의 Tool Search 기능은 이 문제를 도구 정의를 동적으로 로드하는 방식으로 접근하는데, 이것은 MCP를 다루는 글에서 자세히 살펴보겠습니다.

### 설명과 평가의 순환

도구 설명이 프롬프트라면, 프롬프트 엔지니어링과 같은 반복 개선 과정이 필요합니다. 하지만 도구 선택을 평가하는 것은 텍스트 생성을 평가하는 것보다 어렵습니다. "모델이 올바른 도구를 선택했는가?"를 자동으로 판정하려면 별도의 평가 파이프라인이 필요합니다. Anthropic이 제안하는 3단계 개발 프로세스(프로토타입 → 평가 → 에이전트와 협업)는 이 문제에 대한 체계적 접근이며, 평가 체계 설계는 시리즈 뒤쪽에서 다룹니다.

### 비멱등 연산의 현실

이메일 전송, 결제 처리, 외부 API 호출 같은 연산은 본질적으로 비멱등합니다. "취소하고 다시 보내기"가 불가능한 작업에서 에이전트의 재시도는 실제 피해를 만들 수 있습니다. 프로덕션 에이전트가 이런 도구에 확인 게이트(사용자 승인)를 필수로 두는 것은 기술적 한계에 대한 현실적인 해법입니다. 퍼미션과 안전성 설계는 이 시리즈의 뒤쪽에서 깊이 다룹니다.
