...
알고리즘을 아는 것과 프로젝트를 완성하는 것은 완전히 다른 문제다. 이 글에서는 머신러닝 프로젝트가 어떤 단계를 거쳐 완성되는지 전체 흐름을 잡는다. [지난 글에서 지도학습, 비지도학습, 강화학습의 개념을 살펴봤다면](/ml/overview/), 이번엔 "실제로 프로젝트를 어떻게 진행하는가"에 집중한다.
...
다음 글부터는 본격적으로 알고리즘을 파고든다. 가장 기본적인 지도학습 알고리즘인 [선형 회귀(Linear Regression)](/ml/linear-regression/)부터 시작한다. 오늘 살펴본 집값 예측 예시가 실제로 어떻게 작동하는지 수식과 코드로 낱낱이 분해할 예정이다.
