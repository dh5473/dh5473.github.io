...
교차 검증은 데이터를 여러 번 다르게 나눠서 이 문제를 푼다. 그런데 "다르게 나눈다"는 말에는 생각보다 결정할 게 많다. 무작위로 섞을지, 클래스 비율을 맞출지, 시간 순서를 지킬지에 따라 나오는 숫자의 의미가 달라진다.

## Hold-out 한 번은 운에 맡기는 일이다

가장 간단한 평가는 데이터를 훈련셋과 테스트셋으로 한 번 나누는 Hold-out이다. 빠르고 직관적이지만 점수가 크게 흔들린다.

유방암 데이터에서 300건을 뽑아 랜덤 포레스트를 학습시키고, 분할 시드만 바꿔가며 20번 반복해 보면 이렇다.
...
X, _, y, _ = train_test_split(X0, y0, train_size=300, random_state=0, stratify=y0)
...
print(f"{min(scores):.3f} ~ {max(scores):.3f}, 표준편차 {np.std(scores):.3f}")
```

```text
0.900 ~ 0.983, 표준편차 0.028
```

정확도가 8.3%p 폭으로 벌어진다. 같은 데이터, 같은 모델, 같은 코드인데 오직 분할 시드만 달랐다. 테스트셋이 60건이니 어려운 샘플 다섯 개가 어느 쪽에 걸리느냐로 이 폭이 다 만들어진다.

같은 데이터에 `StratifiedKFold(n_splits=5, shuffle=True)`를 걸고 그 평균을 시드별로 재면 폭이 2.0%p, 표준편차가 0.0056으로 줄어든다. 다섯 배 안정된 셈이다. Hold-out은 하나의 분할에서 하나의 점수를 얻지만, K-Fold는 서로 다른 K개의 분할에서 K개를 얻어 평균내기 때문이다.
...
```text
split 1: 훈련 20개 (0~19), 검증 16개 (20~35)
split 2: 훈련 36개 (0~35), 검증 16개 (36~51)
split 3: 훈련 52개 (0~51), 검증 16개 (52~67)
split 4: 훈련 68개 (0~67), 검증 16개 (68~83)
split 5: 훈련 84개 (0~83), 검증 16개 (84~99)
```

검증 구간은 16건으로 고정되고 훈련 구간만 늘어난다. 첫 회차는 전체의 20%만 보고 학습한 모델을 평가하는 셈이라 점수가 낮게 나오기 쉽다. 회차별 점수를 그냥 평균내면 이 초반 핸디캡이 섞여 들어간다.
...
점수를 더 안정시키고 싶으면 K를 키우는 대신 **서로 다른 시드로 K-Fold를 여러 번 반복**하는 게 낫다. 반복마다 폴드 구성이 새로 짜이므로, 서로 덜 겹치는 점수를 더 모으는 셈이다. 앞의 유방암 데이터에서 5-폴드 한 번은 시드에 따라 2.0%p 흔들렸지만, 5-폴드를 10번 반복한 평균은 0.8%p 안에서 움직인다. 모델 A와 B의 0.01 차이가 진짜인지 판단할 때 이 차이가 결정적이다.
...
`x = np.linspace(-1, 1, 100)`에 $y = \sin 3x$ 와 표준편차 0.1 잡음을 얹고 9차 다항 회귀를 걸어보면, `KFold(5)` 기본값의 CV MSE가 993이 나온다. `shuffle=True`만 주면 0.011이다. 다섯 자릿수 차이가 모델이 아니라 정렬 상태에서 왔다. 앞의 숫자는 잡음을 새로 뽑을 때마다 수십에서 수천 사이를 오가는데, 9차 다항식이 훈련 구간 밖으로 얼마나 튀어 오르느냐에 달렸기 때문이다. 시계열이 아니라면 `shuffle=True`를 명시하는 편이 안전하다.
