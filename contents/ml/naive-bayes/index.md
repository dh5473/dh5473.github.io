...
이유는 분류가 확률값 자체를 쓰지 않기 때문이다. 필요한 것은 $P(S \mid x)$와 $P(\neg S \mid x)$ 중 어느 쪽이 큰지, 부등호의 방향 하나뿐이다. 상관된 특성을 독립으로 취급하면 같은 정보가 여러 번 곱해져 확률값이 한쪽으로 부풀지만, 그 부풀림은 대체로 두 클래스에서 비슷한 방향으로 일어나 순위를 뒤집지 못한다. 그래서 나이브 베이즈가 출력하는 확률값은 0.999처럼 극단으로 쏠려 있어 그대로 믿을 것이 못 되지만, 어느 클래스가 이겼는지는 믿을 만하다. "스팸일 확률 82%"처럼 수치를 사용자에게 보여주거나 임계값을 세밀하게 조정해야 하는 자리라면 로지스틱 회귀 쪽이 맞다.
