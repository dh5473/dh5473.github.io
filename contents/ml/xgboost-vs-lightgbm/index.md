...
$T$는 리프 노드 수, $w_j$는 각 리프의 예측값이다. $\gamma$가 크면 분할을 억제하고(가지치기 효과), $\lambda$가 크면 예측값을 보수적으로 줄인다(L2 규제). 과적합 방지가 목적 함수 수준에서 작동하는 셈이다.
...
| 정규화 | 목적함수 내장 ($\gamma$, $\lambda$, $\alpha$) | 외부 파라미터 (`lambda_l1`/`l2`) |
...
XGBoost의 `max_depth=6`은 최대 $2^6 = 64$개 리프를 의미한다. LightGBM에서 같은 복잡도를 원한다면 `num_leaves=63`으로 맞춰야 한다. `max_depth`만 복사하면 의도한 것과 다른 모델이 만들어진다.
