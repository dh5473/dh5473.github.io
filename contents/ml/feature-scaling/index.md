...
[이전 글](/ml/categorical-encoding/)에서 범주형 변수를 숫자로 바꾸는 방법을 정리했다. 원-핫 인코딩, 레이블 인코딩, 빈도 인코딩까지 — 범주형 데이터가 모델에 들어갈 수 있는 형태가 되었다. 그런데 숫자로 바뀌었다고 끝이 아니다. **숫자들의 스케일이 제각각이면** 문제가 생긴다.
...
평균 = 169.2, 표준편차 = 371.6
변환 결과: [-0.45, -0.45, -0.45, -0.44, -0.44, 2.24]
...
StandardScaler: 평균=169.2, 표준편차=371.6 → 이상치가 통계를 지배
...
피처 스케일링은 화려한 기법이 아니다. 공식도 단순하고 코드도 두 줄이면 끝난다. 하지만 이 두 줄이 KNN의 정확도를 0.72에서 0.98로 바꾸고, 경사 하강법의 수렴 속도를 몇 배 빠르게 만든다. 반대로, 테스트 데이터로 `fit`하는 한 줄의 실수가 모델 평가 전체를 무의미하게 만들기도 한다.

결국 스케일링에서 중요한 건 "어떤 스케일러를 쓰느냐"보다 **"왜 이 스케일러를 골랐는지 설명할 수 있느냐"** 다. 이상치가 있는데 StandardScaler를 쓰거나, 트리 모델에 불필요한 스케일링을 적용하는 건 코드가 아니라 판단의 문제다. 그리고 그 판단을 Pipeline으로 구조화해두면, 실수할 여지 자체가 사라진다.
