...
나이는 0에서 100, 연봉은 2,000만에서 2억, 클릭률은 0.001에서 0.05 사이에 있다. 이 셋을 그대로 한 모델에 넣으면 연봉이 나머지 둘을 덮어버린다. 연봉이 더 중요한 피처여서가 아니라 단지 숫자의 자릿수가 커서다. 피처마다 제각각인 값의 범위를 비슷하게 맞추는 작업이 **피처 스케일링(Feature Scaling)** 이다.
...
그 결과 비용 함수의 등고선이 한쪽으로 심하게 늘어난 타원이 된다. 학습률은 하나뿐인데 방향마다 적정 보폭이 다르니, 좁은 축에서는 매번 반대편으로 튕겨 나가고 넓은 축에서는 거의 나아가지 못한다. 최저점에 닿을 때까지 진동만 반복하는 것이다. 스케일을 맞추면 등고선이 원에 가까워지고 어느 방향으로 가든 같은 보폭이 맞아떨어지니, 훨씬 적은 스텝으로 도착한다.
...
값의 범위가 물리적으로 정해져 있을 때 유용하다. 0부터 255까지인 픽셀 값을 0과 1 사이로 옮기거나, 0부터 100까지로 보장된 센서 값을 다룰 때가 그렇다.
...
sklearn의 Wine 데이터셋은 피처 스케일이 대놓고 제각각이다. proline은 278에서 1,680 사이인데 hue는 0.48에서 1.71 사이라, 자릿수가 세 개 차이 난다. 여기에 k=5짜리 KNN을 그대로 돌린 뒤 스케일러만 바꿔봤다.
...
for name, scaler in [('없음', None), ('Standard', StandardScaler()),
    ...
    print(f"{name} | 정확도: {accuracy_score(y_test, knn.predict(X_te)):.4f}")
```
...
배포된 모델에는 데이터가 한 건씩 들어온다. 그 한 건의 평균이나 최댓값 같은 건 존재하지 않는다. 학습 때 계산해둔 통계를 그대로 적용하는 수밖에 없다는 뜻이다. 그런데 평가 단계에서만 테스트 전체의 통계를 써버리면, 실제 배포 환경에서는 불가능한 조건으로 점수를 매기는 것이 된다. 이렇게 매긴 점수는 실제 성능보다 높게 나오고, 얼마나 부풀었는지도 알 길이 없다.
