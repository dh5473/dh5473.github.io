...
sklearn 내부에서도 이 계산, 정확히는 수치적으로 더 안정된 최소제곱 분해를 수행한다.
...
네 가지를 한꺼번에 훑어보는 도구가 **잔차 플롯(Residual Plot)** 이다. 가로축에 예측값, 세로축에 잔차를 찍은 산점도 한 장이면 된다. 잔차가 예측값 전 범위에서 0 주변에 고르게 흩어져 있으면 가정이 충족된 것이다. U자나 역U자가 보이면 관계가 애초에 직선이 아니라는 신호이고, 이때는 다항 회귀나 로그 변환을 검토한다. 오른쪽으로 갈수록 잔차가 벌어지는 부채꼴이면 등분산성이 깨진 것이다.
