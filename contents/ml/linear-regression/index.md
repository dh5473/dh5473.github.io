...
print(f"기울기(w): {model.coef_[0]:.4f}")    # 0.0380
print(f"절편(b): {model.intercept_:.4f}")     # -0.0818
```

`fit()` 한 줄이 전부다. 내부적으로 sklearn은 데이터를 보고 오차를 가장 줄이는 w와 b를 계산한다.

결과를 해석해보면: 기울기가 약 0.038이므로, 면적이 1m² 늘어날 때마다 집값이 약 380만 원 올라간다고 모델이 학습했다. 이제 새로운 면적을 넣으면 가격을 예측할 수 있다.

```python
new_area = np.array([[100]])
predicted = model.predict(new_area)
print(f"100m² 예측 가격: {predicted[0]:.2f}억")  # 3.72억
```

`predict()`가 내부적으로 하는 일은 단순하다. `y = 0.0380 × 100 + (-0.0818) = 3.72`를 계산하는 것이다.
...
```
면적  60m² | 실제: 2.1억 | 예측: 2.20억 | 잔차: -0.10억
면적  75m² | 실제: 2.8억 | 예측: 2.77억 | 잔차: +0.03억
면적  85m² | 실제: 3.2억 | 예측: 3.15억 | 잔차: +0.05억
면적  95m² | 실제: 3.6억 | 예측: 3.53억 | 잔차: +0.07억
면적 110m² | 실제: 4.1억 | 예측: 4.10억 | 잔차: -0.00억
면적 120m² | 실제: 4.5억 | 예측: 4.48억 | 잔차: +0.02억
면적 140m² | 실제: 5.2억 | 예측: 5.24억 | 잔차: -0.04억
면적 155m² | 실제: 5.8억 | 예측: 5.81억 | 잔차: -0.01억
```
...
왜 이렇게 되는가? MSE를 w에 대해 미분하고 0으로 놓으면(= 기울기가 0인 바닥) 이 식이 나온다. XᵀX는 데이터의 특성 간 관계를 담고, Xᵀy는 특성과 정답 간의 관계를 담는다. 역행렬을 곱해 둘을 연결하는 최적의 w를 한 번에 구하는 것이다. numpy로 직접 계산해보면:
...
print(f"편향(b): {w[0]:.4f}")    # -0.0818
print(f"기울기(w): {w[1]:.4f}")  # 0.0380
```
...
<div style="background: #fff3f0; border-left: 4px solid #ff6b6b; padding: 16px 20px; margin: 20px 0; border-radius: 4px;">
  ...
  정규 방정식은 특성 수(p)가 많아질수록 행렬 역행렬 계산이 O(p³)으로 느려진다. 특성이 수만 개 이상이면 정규 방정식보다 <strong>경사하강법(Gradient Descent)</strong>이 훨씬 효율적이다. 이후 글에서 자세히 다룬다.
</div>
...
print(f"기울기(w): {model.coef_[0]:.4f}")                              # 0.4193
print(f"절편(b):   {model.intercept_:.4f}")                            # 0.4446
print(f"R² 스코어: {r2_score(y_test, y_pred):.3f}")                    # 0.459
print(f"RMSE:      {np.sqrt(mean_squared_error(y_test, y_pred)):.3f}") # 0.842
```

R² 스코어가 0.459이다. 중위 소득 하나만으로 집값을 예측했으니 낮지 않은 편이지만, 완벽하지는 않다. 집값에는 위치, 방 수, 건물 연도 등 수많은 요소가 영향을 미치기 때문이다. 특성을 여러 개 사용하는 **다중 선형 회귀(Multiple Linear Regression)** 가 이 문제를 해결한다 — 이후 글에서 다룬다.
...
<div style="background: #f8f9fa; border: 1px solid #e9ecef; padding: 20px; margin: 24px 0; border-radius: 8px;">
  ...
  <ul style="margin: 0; padding-left: 20px;">
    ...
    <li><strong>R² 스코어</strong>: 모델이 데이터의 변동성을 얼마나 설명하는지 (1에 가까울수록 좋음, 음수도 가능)</li>
...
다음 글에서는 "잔차를 최소화한다"는 말을 수학적으로 정확하게 정의하는 **[비용 함수(Cost Function)](/ml/cost-function/)** 를 다룬다. 왜 잔차를 제곱하는지, 왜 평균을 취하는지, 이 이유를 이해하고 나면 이후 경사하강법과 신경망으로 이어지는 흐름이 자연스럽게 보이기 시작한다.
