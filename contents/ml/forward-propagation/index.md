...
예측이 0.5000이고 정답은 1이니, 모델이 틀렸다. Loss가 0.6931(= ln2)로 꽤 높다. 히든2 출력이 전부 0인 것은 초기 가중치가 좋지 않아 ReLU가 모든 뉴런을 죽인 것이다 — 이런 "dying ReLU" 현상은 [활성화 함수](/ml/activation-functions/) 글에서 다룬다. 이제 이 Loss를 줄이기 위해 W와 b를 업데이트해야 하는데 — 그게 바로 역전파다.
