...
왜 이렇게 되는가? 합성함수의 미분(chain rule)이다. (wxᵢ + b − yᵢ)²을 w로 미분하면, 제곱의 지수 2가 앞으로 나오고 안쪽을 w로 미분한 xᵢ가 곱해진다. b로 미분하면 안쪽의 b 미분이 1이므로 xᵢ가 사라진다.
...
```
Epoch     0 | Cost: 16.648750 | w: 0.008920 | b: 0.000078
...
최종 결과: w = 0.0373, b = -0.0003
```

5000번 반복 후 w ≈ 0.037로 최적값(0.0380)에 가까워졌지만, b는 -0.0003으로 최적값(-0.0818)에 한참 못 미친다. w는 빠르게 수렴하는데 b가 느린 이유는, 입력값(면적)의 스케일이 60~155로 크기 때문에 w의 기울기(`dw = (2/n) × Σ(error × X)`)가 b의 기울기(`db = (2/n) × Σ(error)`)보다 수십 배 크기 때문이다. 하나의 학습률로 둘 다 업데이트해야 하므로, dw에 맞춰 학습률을 작게 잡으면 db 업데이트가 너무 작아진다. 이게 바로 **Feature Scaling**이 중요한 이유다 — [다중 선형 회귀 글](/ml/multiple-linear-regression/)에서 이 문제를 정면으로 해결한다.

<div style="background: #f0fff4; border-left: 4px solid #51cf66; padding: 16px 20px; margin: 20px 0; border-radius: 4px;">
  <strong>✅ 정규 방정식 vs 경사하강법</strong><br>
  정규 방정식은 한 번에 정확한 해를 구하지만, 특성 수가 많아질수록 행렬 역행렬 계산이 O(p³)으로 느려진다. 경사하강법은 반복이 필요하지만 특성 수에 관계없이 효율적이다. 특성이 수만 개 이상인 현실 문제에서는 경사하강법이 사실상 유일한 선택지다.
</div>
...
<p align="center" style="color: var(--text-muted, #6d6762); font-size: 13px;"><em>학습률이 너무 작으면 수렴이 느리고, 너무 크면 발산한다. 적절한 학습률에서 효율적으로 수렴한다.</em></p>
...
이 시리즈에서 ML의 핵심 흐름을 따라왔다. [데이터에 직선을 맞추고](/ml/linear-regression/), [오차를 측정하는 기준을 정의하고](/ml/cost-function/), 그 오차를 줄이는 알고리즘을 이해했다. 이 세 가지 — 모델, 비용 함수, 최적화 — 가 ML의 뼈대다.

다음 글에서는 변수를 여러 개로 확장하는 **[다중 선형 회귀](/ml/multiple-linear-regression/)** 와, 이 글에서 겪었던 수렴 문제를 근본적으로 해결하는 **Feature Scaling**을 다룬다.
