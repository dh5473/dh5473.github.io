...
이 중 어느 하나를 감으로 고르면 대부분 시간을 버린다. 데이터가 부족한 게 아닌데 라벨링에 두 주를 쓰고, 이미 과적합인 모델을 더 키우고, 노이즈뿐인 피처를 더 넣는다. 다섯 개 선택지는 서로 배타적이기까지 하다. 편향이 높을 때 맞는 처방과 분산이 높을 때 맞는 처방이 정확히 반대라서, 잘못 고르면 제자리가 아니라 뒤로 간다.
...
**진단 없이 튜닝하면 정확히 반대로 간다**

위 표의 두 열은 방향이 모두 반대다. 학습 곡선을 보지 않고 감으로 손대면 과소적합 모델에 규제를 더 걸거나, 과적합 모델에 피처를 더 넣는 일이 생긴다. 시간만 쓰고 점수는 오히려 내려간다.
...
전통적인 워크플로는 데이터를 고정해두고 모델을 바꾼다. Andrew Ng이 제안한 데이터 중심(Data-Centric) 접근은 반대로 모델을 고정하고 데이터 품질을 체계적으로 손본다. 라벨 정제, 유형별 보강, 애매한 사례의 정의를 다시 잡는 일이 여기 들어간다. 실무에서 점수를 가장 크게 움직이는 레버는 대체로 모델이 아니라 데이터 쪽에 있다.

:::

학습 곡선이 "데이터가 더 필요하다"고 말할 때도 오차 분석이 방향을 정해준다. 전체 데이터를 무작위로 10% 늘리는 것보다, 비영어 스팸처럼 모델이 집중적으로 틀리는 유형만 골라 모으는 쪽이 훨씬 싸게 먹힌다.

데이터를 새로 모으기 어려우면 증강을 쓴다. 이미지는 회전·반전·크롭·색상 변환, 텍스트는 동의어 대체와 역번역이 표준이다. 정형 데이터에서는 SMOTE 같은 오버샘플링이 있지만, 이미지나 텍스트만큼 효과를 보기는 어렵다. 픽셀을 뒤집어도 고양이는 고양이지만, 수치 피처를 흔들면 그 행이 현실에 존재할 수 있는 조합인지 보장할 수 없기 때문이다.
...
정형 데이터에서 트리 기반 부스팅이 앞서는 이유는 피처 스케일에 둔감하고, 결측값 처리가 내장되어 있고, 학습이 빠르다는 세 가지다. 딥러닝이 확실히 앞서는 곳은 이미지·텍스트·음성처럼 원시 입력에서 표현을 직접 배워야 하는 문제다.
...
점수가 이상할 때 순서대로 확인할 목록이다.

- 전처리가 분할보다 먼저 실행되지 않는가
- 피처에 정답에서 파생된 값이 섞여 있지 않은가
- 시계열인데 무작위로 나누지 않았는가
- 지표가 문제에 맞는가
...
"다음에 뭘 해야 하는가"는 취향의 문제가 아니라 측정의 문제다. 학습 곡선은 두 곡선의 높이와 간격으로 편향인지 분산인지를 가르고, 인간 수준 성능이 있으면 그 둘을 퍼센트로 쪼개준다. 오차 분석은 그다음 질문인 "어느 유형부터 손댈 것인가"에 상한선이 붙은 답을 준다. 이 두 도구가 있으면 개선 후보 다섯 개 중 넷은 지워진다.

기억할 것은 편향과 분산의 처방이 서로 반대라는 점이다. 규제를 올릴지 내릴지, 모델을 키울지 줄일지, 데이터를 더 모을지 말지가 전부 진단에 달려 있다. 진단을 건너뛴 튜닝은 절반의 확률로 뒷걸음질이다.
