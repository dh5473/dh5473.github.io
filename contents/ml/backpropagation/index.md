...
| 항 | 무엇을 미분한 것인가 | 값 |
|---|---|---|
| $\partial L / \partial a_2$ | $L = (y-a_2)^2$를 $a_2$로 | $-2(1 - a_2) = -0.8223$ |
...
순전파와 역전파는 같은 계산 그래프를 반대 방향으로 한 번씩 훑는다. 앞으로 갈 때는 값이 흐르고, 뒤로 올 때는 기울기가 흐른다.
...
$n^{[l]}$은 층 $l$의 뉴런 수, $m$은 배치 안의 샘플 수다. $dW$가 $W$와 같은 크기로 나오는지 확인하는 것만으로도 전치를 빠뜨렸거나 축을 잘못 잡은 실수는 대부분 걸러진다.
...
층을 하나 거칠 때마다 $g'(z)$가 한 번씩 곱해진다. 시그모이드 도함수의 최댓값은 $z=0$일 때의 $0.25$이고 그 바깥에서는 더 작으니, 곱해지는 수가 아무리 커도 4분의 1이다.
...
반대쪽으로 터지는 경우도 있다. $W$의 원소가 크면 $W^\top$을 곱할 때마다 기울기가 커져서, 층마다 2배씩만 불어나도 10층이면 1024배가 된다. 업데이트 폭이 폭발하면서 손실이 발산하고 곧 NaN이 뜬다. 학습 중 갑자기 NaN을 보면 십중팔구 **기울기 폭발(exploding gradient)** 이다.
...
역전파는 이름 그대로 방향에 관한 알고리즘이다. 연쇄 법칙은 어느 쪽에서 세워도 같은 답을 주지만, 출력 쪽에서 세워야 앞 층이 필요로 하는 항이 이미 손에 있다. 새로운 수학이 아니라 계산 순서가 이 알고리즘의 내용이다.
...
- [CS231n, Backpropagation and Intuitions](https://cs231n.github.io/optimization-2/)
...
- [PyTorch, A Gentle Introduction to torch.autograd](https://pytorch.org/docs/stable/notes/autograd.html)
