...
MAE(0.4000)와 비슷하지만 약간 더 크다(0.4195). 이건 항상 그렇다 — 수학적으로 **RMSE >= MAE**가 보장된다. 제곱 → 평균 → 루트 과정에서 큰 오차의 비중이 높아지기 때문이다.
...
이상치 포함  → MAE: 0.6800, RMSE: 1.3446, 비율: 1.98
```
...
RMSE(23.77)가 MAE(17.00)보다 크다. 300을 350으로 예측한 오차(50)가 제곱 과정에서 지배적으로 작용했기 때문이다.
...
MAE=14만으로는 좋은지 나쁜지 판단하기 어렵다. MAPE=5.47%면 "평균적으로 약 5% 오차"라고 바로 해석된다.
