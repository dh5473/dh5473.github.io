...
보통 5~50 범위에서 시작하되, 정답은 없다. 데이터가 크면 약간 높여볼 수 있지만, 100을 넘기는 경우는 드물다. **여러 값을 시도해보는 것**이 최선이다.
...
UMAP은 **위상수학(topology)** 에 기반한다. 직관적으로, 데이터가 구겨진 종이 위에 놓여 있다고 가정하고, UMAP은 이 종이를 펼쳐서 평면에 펼치려 하는 것이다. 수학적으로는 두 가지 가정을 둔다.

1. **데이터는 고차원 공간에 균일하게 분포한 다양체(manifold) 위에 놓여 있다.**
2. **이 다양체의 거리 구조는 국소적으로 일정하다.** (가까운 점들 사이의 거리 척도가 위치마다 크게 변하지 않는다는 뜻이다)
