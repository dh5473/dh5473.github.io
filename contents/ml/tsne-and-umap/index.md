...
이 목적함수의 생김새가 t-SNE의 성질을 거의 다 결정한다. $p_{ij}$ 가 큰데 $q_{ij}$ 가 작으면 로그 항이 커져 벌점이 크다. 고차원에서 가까웠던 점을 저차원에서 떼어놓는 실수는 크게 벌한다는 뜻이다. 반대로 $p_{ij}$ 가 작으면 앞에 곱해진 가중치 자체가 작아서 $q_{ij}$ 가 얼마든 벌점이 거의 없다. **멀리 있던 점을 가깝게 그려도 손해가 별로 없다.**
...
축의 방향도 마찬가지다. 목적함수가 점들 사이의 거리만 보기 때문에 임베딩을 통째로 회전시켜도 손실이 그대로다. x축이 무엇을 뜻하느냐는 질문 자체가 성립하지 않는다.
...
64차원이 2차원 평면에서 열 덩어리로 갈린다. 평면 위에서 각 점의 최근접 이웃 열 개를 세어보면 같은 숫자가 92%에서 100%를 차지하고, 섞이는 나머지는 1과 8, 5와 9, 3과 9 같은 몇 쌍에 몰려 있다.
...
샘플이 수만을 넘어가면 속도만으로도 UMAP 쪽이 현실적인 선택이 되고, 임베딩을 재사용해야 하는 자리에서는 sklearn만 놓고 볼 때 UMAP뿐이다. 다만 UMAP의 전역 구조가 "상대적으로 낫다"는 것이지 군집 사이 거리를 그대로 믿어도 된다는 뜻은 아니다. 거리를 정량적으로 논해야 하는 자리에서는 두 방법 모두 근거가 되지 못한다.

## PCA와는 역할이 다르다

셋을 나란히 놓고 하나를 고르는 문제가 아니다. PCA는 전처리 도구고, t-SNE와 UMAP은 그림 그리는 도구다.

PCA는 매번 같은 결과를 내고, 역변환이 되고, 각 축이 원본 피처의 선형 결합이라 의미를 따질 수 있다. 그래서 모델 파이프라인 안에 넣어도 된다. 반면 t-SNE는 시드를 고정하지 않으면 돌릴 때마다 좌표가 달라지고, 고정하더라도 새 데이터를 받지 못한다. UMAP은 새 데이터를 받긴 하지만 축 자체에 해석할 의미가 없다.
...
X_2d = umap.UMAP().fit_transform(X_50)
```

그림에서 군집이 보인다고 해서 그것을 클러스터링 결과로 쓰면 안 된다. 두 알고리즘은 애초에 덩어리를 만들도록 설계되어 있어서, 구조가 전혀 없는 데이터를 넣어도 그럴듯한 군집을 그려낸다. 64차원 가우시안 노이즈 1,000개를 t-SNE에 넣고 나온 평면을 K-Means로 여덟 덩어리로 자르면 실루엣 점수가 0.32다. 같은 데이터를 원본 64차원에서 똑같이 자르면 0.01로, 나눌 것이 없다는 정직한 값이 나온다. 실제 군집은 원본 공간에서 K-Means나 DBSCAN으로 찾고, 그림은 그 결과를 눈으로 확인하는 용도로 쓴다.
...
두 알고리즘의 차이는 목적함수 한 줄에서 갈린다. t-SNE의 KL 발산은 가까운 것을 떼어놓는 실수만 벌하고, UMAP의 교차 엔트로피는 먼 것을 붙여놓는 실수도 벌한다. UMAP의 전역 배치가 조금 더 믿을 만한 이유가 여기 있다. 새 데이터를 기존 좌표계에 얹을 수 있는 것은 목적함수와 별개로, 학습한 이웃 그래프를 그대로 두고 새 점의 자리만 찾는 구현이 붙어 있어서다.

그리고 이 둘은 PCA를 대체하지 않는다. 모델에 넣을 차원 축소는 PCA고, 사람이 볼 그림은 t-SNE와 UMAP이다. 이 경계를 넘어 t-SNE 좌표를 피처로 넘기는 순간, 샘플이 하나 늘 때마다 전체를 다시 돌려야 하고 그때마다 좌표계가 갈아엎어지는 파이프라인이 만들어진다.
...
- [van der Maaten & Hinton, Visualizing Data using t-SNE (JMLR 2008)](https://www.jmlr.org/papers/v9/vandermaaten08a.html)
- [McInnes et al., UMAP: Uniform Manifold Approximation and Projection for Dimension Reduction (arXiv 2018)](https://arxiv.org/abs/1802.03426)
...
- [umap-learn, Understanding UMAP parameters](https://umap-learn.readthedocs.io/en/latest/parameters.html)
