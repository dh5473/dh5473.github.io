...
사인 곡선에 표준편차 0.3짜리 노이즈를 얹은 점 18개(`np.random.RandomState(21)`)에 다항 회귀를 맞춰 보자. 차수만 바꾸고 나머지는 전부 같다.

<div style="margin: 24px 0; text-align: center;">
<svg viewBox="0 0 400 420" style="width: 100%; height: auto; max-width: 380px;"
     xmlns="http://www.w3.org/2000/svg"
     font-family="Pretendard, -apple-system, sans-serif"
     role="img" aria-label="같은 데이터 18개에 차수 1, 3, 12의 다항 회귀를 맞춘 세 패널. 차수 1은 직선이라 사인 곡선을 전혀 따라가지 못하고, 차수 3은 실제 함수와 거의 겹치며, 차수 12는 점 하나하나를 따라가느라 구불구불하다. 훈련 MSE는 각각 0.35, 0.05, 0.02이다.">
<style>
...
<text class="bv1-s" x="52" y="114">훈련 MSE 0.35</text>
<!-- 패널 2: 차수 3 -->
...
<text class="bv1-s" x="52" y="242">훈련 MSE 0.05</text>
<!-- 패널 3: 차수 12 -->
...
위 패널은 두 곡선이 목표 수준보다 한참 높은 자리에서 만난다. 데이터를 더 넣어도 만나는 높이가 내려가지 않는다. 모델이 표현할 수 있는 함수의 집합 안에 정답이 없기 때문이고, 데이터는 그 집합을 넓혀주지 못한다. 앞의 차수 1 실험이 정확히 이 모양이다. 같은 방식으로 표본만 50개에서 5만 개로 천 배 늘리면 훈련 MSE의 평균은 0.273에서 0.286으로 오히려 조금 올라간다. 직선으로는 좁힐 수 없는 편향 0.196에 노이즈 분산 0.09를 더한 0.286이 이 모델의 바닥이라, 표본이 늘수록 그 바닥에 붙는 것이다.
...
분산 쪽 처방에서 규제가 앞자리에 있는 이유는 복잡도를 깎지 않고도 듣기 때문이다. 위 `decompose` 에서 `LinearRegression()` 만 `Ridge(alpha=1)` 로 바꿔 차수 11을 다시 재보면, 분산이 11.4075에서 0.1097로 백 분의 일이 되고 편향 제곱도 0.0348에서 0.0015로 함께 내려간다. 차수는 11 그대로인데 계수 크기를 눌러 훈련점 사이의 요동만 걷어낸 결과다. 다만 총 오차는 0.2011에 머물러 차수 3의 0.1021에는 끝내 못 미친다. 규제는 과한 복잡도를 사후에 다듬는 도구이지 맞는 복잡도를 고르는 일을 대신해주지는 않는다.
...
규제는 복잡도 손잡이를 왼쪽으로 미는 일이다. 너무 밀면 반대편 낭떠러지로 떨어진다. 차수 4 다항 회귀에 Ridge 강도만 바꿔가며 같은 방식으로 총 오차를 재보면 이렇다.

| alpha | 없음 | 0.1 | 1 | 100 | 10000 |
|---|---|---|---|---|---|
| 총 오차 | 0.1047 | 0.1067 | 0.1173 | 0.1444 | 0.3084 |

차수 4는 이 데이터에 이미 알맞은 크기라 규제를 걸 여유분이 없다. alpha를 올리는 족족 편향만 늘고, 10000에서는 총 오차가 0.3084로 차수 1 직선의 0.2609보다도 나빠진다. 계수를 끝까지 누르면 4차 다항식이 직선을 지나 거의 평평한 상수 쪽으로 끌려가기 때문이다. 규제 강도도 다른 하이퍼파라미터와 똑같이 검증으로 골라야 한다.
