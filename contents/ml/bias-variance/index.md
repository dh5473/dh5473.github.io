...
[이전 글](/ml/regularization/)에서 규제(Regularization)를 통해 가중치를 억제하면 과적합을 막을 수 있다는 걸 배웠다. 하지만 과적합은 왜 일어나는 걸까? 모델이 복잡해지면 훈련 데이터를 거의 완벽하게 외우지만, 새 데이터에는 형편없어진다 — 이게 **과적합(Overfitting)** 이다. 반대로 너무 단순한 모델은 훈련 데이터조차 제대로 못 맞춘다 — **과소적합(Underfitting)** 이다.
