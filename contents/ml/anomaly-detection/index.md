...
평가는 마지막까지 발목을 잡는다. 이상 탐지에서 정확도는 거의 항상 99%를 넘고, 그 숫자를 보고하는 순간 아무것도 말하지 않은 것과 같아진다. 정밀도와 재현율을 함께 놓고, 어느 쪽을 얼마나 포기할지 도메인 비용으로 정하고, PR-AUC로 임계값과 무관한 성능을 확인한다. 정상을 잘 모델링하는 것이 절반이고, 그 결과를 제대로 재는 것이 나머지 절반이다.
