...
| 공식 | 수식 |
|---|---|
| **조건부 확률** | $P(A \mid B) = \dfrac{P(A \cap B)}{P(B)}$ |
| **곱셈 규칙** | $P(A \cap B) = P(A \mid B) \, P(B) = P(B \mid A) \, P(A)$ |
| **독립** | $P(A \cap B) = P(A) \, P(B) \iff P(A \mid B) = P(A)$ |
| **전확률 공식** | $P(A) = \sum_i P(A \mid B_i) \, P(B_i)$ |
| **베이즈 정리** | $P(A \mid B) = \dfrac{P(B \mid A) \, P(A)}{P(B)} = \dfrac{\text{Likelihood} \times \text{Prior}}{\text{Evidence}}$ |
