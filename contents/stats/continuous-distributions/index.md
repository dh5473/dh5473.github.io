...
scipy로 네 가지 설정의 기댓값, 분산, 그리고 $P(-1 \leq X \leq 1)$을 계산해 비교하면 다음과 같다.
...
scipy로 `rv.cdf(k) - rv.cdf(-k)`를 계산해 보면 $k = 1, 2, 3$에서 정확히 68.27%, 95.45%, 99.73%가 나온다.
...
기댓값과 분산은 위 표에 있으니, 분포의 형태를 결정하는 왜도(Skewness)와 초과 첨도(Excess Kurtosis)만 추가로 비교하면 다음과 같다. scipy에서는 `rv.stats(moments='sk')` 한 줄로 얻을 수 있다.
...
지수·감마 분포의 양의 왜도(오른쪽 꼬리)와 균일 분포의 음의 첨도(꼬리 없음)가 수치로 드러난다.
...
실제로 scipy로 확인해 보면 Uniform(0,1)과 Beta(1,1)의 PDF는 완전히 일치하고, Exp($\lambda$)와 Gamma(1, $1/\lambda$)의 PDF도 마찬가지다. Binomial(1000, 0.3)과 그 정규 근사의 CDF 최대 차이는 약 0.016에 불과하다.
