...
표본 평균이나 표본 분산은 직관적이었다. 모집단 평균을 추정하니까 데이터의 평균을 구하고, 모집단 분산을 추정하니까 데이터의 산포를 계산한다. 그런데 감마 분포의 형태 모수(shape parameter)를 추정해야 한다면? 직관만으로는 부족하다. 임의의 확률 모형에서 모수를 체계적으로 추정하는 일반적인 방법론이 필요하다.
...
$k$차 모집단 적률은 $\mu_k = E[X^k]$이고, 이에 대응하는 $k$차 표본 적률은 $\hat{\mu}_k = \frac{1}{n}\sum_{i=1}^{n}X_i^k$이다. 추정해야 할 모수가 $p$개라면, $p$개의 적률 방정식을 세워서 풀면 된다.

1차 적률 $E[X]$는 모집단 평균이고, 이를 표본 평균 $\bar{X}$로 대체한다. 2차 적률 $E[X^2]$도 마찬가지다. [큰 수의 법칙](/stats/lln-and-clt/)에 의해 $\hat{\mu}_k \xrightarrow{P} \mu_k$이므로, 이 대체는 $n$이 커지면 정확해진다.

### 예시 1: 정규분포 $N(\mu, \sigma^2)$

모수가 2개이므로 적률 방정식도 2개다.
...
정규분포에서는 적률법 추정량이 직관적인 표본 평균, 표본 분산과 정확히 일치한다.
...
주목할 점은 적률법이 $n$으로 나눈다는 것이다. [베셀 보정](/stats/point-estimation/)에서 봤듯 이 추정량은 $\sigma^2$에 대해 편향되어 있지만, 적률법은 원래 그런 것이다 — 표본 적률을 모집단 적률에 그대로 대입하는 방법이니까.

### 예시 2: 감마분포 $\text{Gamma}(\alpha, \beta)$

적률법의 진가는 모수와 적률의 관계가 비선형일 때 드러난다. 감마분포의 평균과 분산은:
...
여기서 $\hat{\mu}_2 = \frac{1}{n}\sum X_i^2$이다. 분모 $\hat{\mu}_2 - \bar{X}^2$은 사실 $n$으로 나눈 표본 분산과 같다.
...
# 1차 표본 적률: 1.4827
# 2차 표본 적률: 2.7705
# MoM α̂ = 3.8428  (참값: 3.0)
# MoM β̂ = 2.5918  (참값: 2.0)
```

감마 분포의 형태 모수를 추정하는 것은 직관만으로는 어렵다. 하지만 적률법은 기계적으로 방정식을 세우고 풀면 된다. 이것이 적률법의 장점이다.

<div style="background: #f0f4ff; border-left: 4px solid #3182f6; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>💡 적률법의 장단점</strong><br>
<strong>장점</strong>: 계산이 간단하다. 우도 함수를 구할 필요 없이 적률만 맞추면 된다. 닫힌 해(closed-form)가 존재하는 경우가 많다.<br>
<strong>단점</strong>: 일반적으로 MLE보다 효율이 낮다(분산이 크다). 고차 적률을 사용하면 추정 불안정성이 커진다. 추정값이 모수 공간 밖에 떨어질 수 있다(예: 분산 추정이 음수).</div>
...
형태는 결합 확률(밀도)함수와 동일하지만, 관점이 다르다. 확률함수는 $\theta$를 고정하고 $x$의 함수로 보지만, 우도 함수는 **$x$를 고정(관측값)하고 $\theta$의 함수**로 본다.

동전 던지기로 직관을 잡아보자. 동전을 10번 던져 앞면이 8번 나왔다.

- $p = 0.5$일 때 우도: $L(0.5) = \binom{10}{8}(0.5)^{10} \approx 0.044$
- $p = 0.8$일 때 우도: $L(0.8) = \binom{10}{8}(0.8)^8(0.2)^2 \approx 0.302$

$p = 0.8$이 훨씬 그럴듯하다. 이 우도를 최대화하는 $p$를 찾는 것이 MLE다.

### 로그우도 (Log-Likelihood)

곱셈을 덧셈으로 바꾸기 위해 로그를 취한다. 로그는 단조증가 함수이므로 최대화 문제의 해가 바뀌지 않는다.

$$\ell(\theta) = \log L(\theta) = \sum_{i=1}^{n} \log f(X_i; \theta)$$

로그우도를 쓰면:
- 곱셈이 덧셈으로 변한다 → 미분이 쉬워진다
- 매우 작은 확률들의 곱이 언더플로를 일으키지 않는다

MLE는 로그우도를 최대화하는 $\theta$:

$$\hat{\theta}_{\text{MLE}} = \arg\max_\theta \ell(\theta)$$

대부분의 경우 미분해서 0으로 놓고 풀면 된다:

$$\frac{\partial \ell}{\partial \theta} = 0 \quad \text{(Score equation)}$$

이 미분 $\frac{\partial \ell}{\partial \theta}$을 **스코어 함수(Score Function)**라 부른다. 스코어 함수의 기댓값은 항상 0이라는 성질이 있다: $E\left[\frac{\partial}{\partial\theta}\log f(X;\theta)\right] = 0$.
...
# n = 50, 성공 횟수 = 35
# MLE p̂ = 0.7000  (참값: 0.7)
```

### 정규분포 $N(\mu, \sigma^2)$

두 모수를 동시에 추정한다. 로그우도는:
...
MLE의 $\hat{\sigma}^2$는 $n$으로 나눈다. 비편향 추정량($n-1$로 나눔)이 아니다. MLE가 항상 비편향은 아니라는 점을 보여주는 중요한 예시다.
...
포아송에서도 MLE는 표본 평균이다. 포아송 분포의 평균이 $\lambda$이므로, 직관적으로도 자연스럽다.
...
지수분포의 평균이 $1/\lambda$이므로, 표본 평균의 역수가 MLE가 된다.

<div style="background: #f8f9fa; border: 1px solid #e9ecef; padding: 20px; margin: 24px 0; border-radius: 8px;"><strong>📌 분포별 MLE 요약</strong><br><br>
<table style="width:100%; border-collapse:collapse;">
<tr style="border-bottom:1px solid #e9ecef;"><td style="padding:8px;"><strong>분포</strong></td><td style="padding:8px;"><strong>모수</strong></td><td style="padding:8px;"><strong>MLE</strong></td><td style="padding:8px;"><strong>비편향?</strong></td></tr>
<tr style="border-bottom:1px solid #e9ecef;"><td style="padding:8px;">Bernoulli($p$)</td><td style="padding:8px;">$p$</td><td style="padding:8px;">$\bar{X}$</td><td style="padding:8px;">✓</td></tr>
<tr style="border-bottom:1px solid #e9ecef;"><td style="padding:8px;">Normal($\mu, \sigma^2$)</td><td style="padding:8px;">$\mu$</td><td style="padding:8px;">$\bar{X}$</td><td style="padding:8px;">✓</td></tr>
<tr style="border-bottom:1px solid #e9ecef;"><td style="padding:8px;">Normal($\mu, \sigma^2$)</td><td style="padding:8px;">$\sigma^2$</td><td style="padding:8px;">$\frac{1}{n}\sum(X_i-\bar{X})^2$</td><td style="padding:8px;">✗ (편향: $\frac{n-1}{n}\sigma^2$)</td></tr>
<tr style="border-bottom:1px solid #e9ecef;"><td style="padding:8px;">Poisson($\lambda$)</td><td style="padding:8px;">$\lambda$</td><td style="padding:8px;">$\bar{X}$</td><td style="padding:8px;">✓</td></tr>
<tr><td style="padding:8px;">Exponential($\lambda$)</td><td style="padding:8px;">$\lambda$</td><td style="padding:8px;">$1/\bar{X}$</td><td style="padding:8px;">✗</td></tr>
</table>
</div>
...
MLE가 적률법보다 선호되는 이유는 **대표본에서의 최적 성질** 때문이다. $n$이 충분히 크면 MLE는 세 가지 놀라운 성질을 갖는다.

### 1. 일치성 (Consistency)

$$\hat{\theta}_{\text{MLE}} \xrightarrow{P} \theta_0 \quad (n \to \infty)$$

MLE는 [일치추정량](/stats/point-estimation/)이다. 데이터가 많아지면 참값에 수렴한다. 정규 조건(regularity conditions) 하에서 성립한다.

### 2. 점근 정규성 (Asymptotic Normality)

$$\sqrt{n}(\hat{\theta}_{\text{MLE}} - \theta_0) \xrightarrow{d} N\left(0, \frac{1}{I(\theta_0)}\right)$$

$n$이 커지면 MLE의 분포는 정규분포에 가까워진다. 여기서 $I(\theta_0)$는 **피셔 정보량(Fisher Information)**이다. 이 결과가 신뢰구간 구성의 기반이 된다 — 다음 글의 주제이기도 하다.

실제로 확인해보자. 포아송 분포의 MLE인 $\hat{\lambda} = \bar{X}$가 $n$이 커질수록 정규분포에 얼마나 가까워지는지 시뮬레이션한다.
...
실증 분산이 이론값 $\lambda/n = 1/(nI(\lambda))$에 거의 정확히 일치한다. 분산의 비율이 1에 가깝다는 것은 MLE가 [크래머-라오 하한](/stats/point-estimation/)에 도달하고 있다는 뜻이다.

### 3. 점근 효율성 (Asymptotic Efficiency)

MLE의 점근 분산 $1/(nI(\theta))$은 크래머-라오 하한과 일치한다. 이 말은 **MLE보다 분산이 작은 일치추정량은 (점근적으로) 존재하지 않는다**는 뜻이다.
...
<div style="background: #f0fff4; border-left: 4px solid #51cf66; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>✅ 실전 의미</strong><br>MLE의 점근 성질은 "데이터가 충분하면 MLE가 최적에 가깝다"고 요약할 수 있다. ML에서 로지스틱 회귀의 손실함수(cross-entropy)가 사실 MLE이고, <a href="/stats/information-theory/">교차 엔트로피</a>와 우도 최대화가 같은 문제라는 것도 이 맥락에서 이해할 수 있다.</div>
...
[이전 글](/stats/point-estimation/)에서 크래머-라오 하한을 소개하면서 피셔 정보량을 간략히 언급했다. 이제 더 깊이 들어가보자.
...
두 번째 형태가 직관적이다. 로그우도의 **곡률(curvature)**이 클수록 피셔 정보가 크다. 곡률이 크다는 것은 우도 함수가 참값 근처에서 뾰족하다는 뜻이고, 그만큼 모수의 위치를 정밀하게 특정할 수 있다.
...
베르누이의 $I(p) = 1/(p(1-p))$를 보자. $p = 0.5$일 때 $I(p) = 4$로 최소다. $p$가 0이나 1에 가까울수록 피셔 정보가 커진다. 직관적으로, 동전이 거의 항상 앞면(또는 뒷면)이 나오면 $p$의 값을 적은 시행으로도 정확하게 추정할 수 있다. 반면 $p = 0.5$인 공정한 동전은 가장 불확실하므로 정밀한 추정에 더 많은 데이터가 필요하다.

---

## MoM vs MLE: 언제 무엇을 쓸까?

감마 분포의 형태 모수 $\alpha$를 추정하는 시뮬레이션으로 두 방법을 직접 비교해보자.
...
MLE가 MoM보다 편향도 작고 분산도 작다. MSE 기준으로 약 32% 더 정확하다. $n$이 작을수록 이 차이가 두드러진다.
...
<div style="background: #f0f4ff; border-left: 4px solid #3182f6; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>💡 실전에서의 선택</strong><br>대부분의 경우 MLE를 쓴다. 특히 ML에서는 거의 모든 학습이 우도 최대화(= 손실 최소화)로 귀결된다. 로지스틱 회귀의 <a href="/stats/information-theory/">교차 엔트로피 손실</a>이 바로 음의 로그우도다. 적률법은 MLE의 초기값으로 쓰거나, 우도 함수를 모르는 경우(비모수적 상황)에서 유용하다.</div>
...
정규분포 MLE는 $n$으로 나누므로 비편향이 아니다. 하지만 [MSE 분해](/stats/point-estimation/)에서 봤듯이, 편향된 MLE가 비편향 추정량보다 MSE가 작을 수 있다. 소표본에서 비편향성이 중요하면 $n-1$로 나눈 $S^2$를, 대표본이면 MLE든 비편향이든 차이가 무시할 수준이다.

### 2. 우도 함수가 다봉(multimodal)일 수 있다

혼합 분포 등에서는 로그우도 함수에 여러 극값이 존재할 수 있다. 이 경우 그래디언트 기반 최적화가 지역 최대(local maximum)에 빠질 수 있다. EM 알고리즘이나 다중 초기값 전략으로 대응한다.

### 3. 적률법 추정값이 모수 공간 밖으로 나갈 수 있다

감마 분포에서 표본 분산이 우연히 매우 작으면 $\hat{\alpha}$가 비현실적으로 커질 수 있다. MLE는 보통 이런 문제가 덜하다 — 최적화 자체가 모수 공간 내에서 이루어지기 때문이다.

### 4. 정규 조건이 만족되지 않으면 점근 성질이 무너진다

MLE의 점근 성질이 성립하려면 정규 조건(regularity conditions)이 필요하다:
- 모수 공간이 열린 집합
- 서포트(support)가 $\theta$에 의존하지 않음
- 로그우도가 3번 미분 가능

이 조건이 깨지는 대표적 예가 균일분포 $\text{Uniform}(0, \theta)$다. 서포트가 $\theta$에 의존하므로 MLE($\hat{\theta} = X_{(n)}$, 최댓값)의 수렴 속도가 $1/\sqrt{n}$이 아니라 $1/n$이다.

<div style="background: #fff3f0; border-left: 4px solid #ff6b6b; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>⚠️ 주의</strong><br>MLE가 "항상 최선"이라는 인식은 점근적 결과에 기반한 것이다. $n$이 작으면 MLE도 편향될 수 있고, 정규 조건이 깨지면 일치성조차 보장되지 않는다. 소표본에서는 추정량의 유한표본 성질을 직접 확인하는 것이 중요하다.</div>

---

## 마치며

적률법은 "적률을 맞춘다"는 단순한 원리로 어떤 분포에서든 기계적으로 추정량을 구성할 수 있는 방법이다. MLE는 "데이터의 우도를 최대화한다"는 원리로, 점근적으로 가장 효율적인 추정량을 제공한다. 대부분의 실전 상황에서 MLE가 선호되고, 특히 ML의 손실 함수 설계 전체가 MLE 프레임워크 위에 서 있다.

그런데 MLE가 제공하는 것은 결국 하나의 숫자(점추정값)다. "참값은 이 근처에 있다"는 정보는 있지만, **얼마나 근처인지**는 알 수 없다. 다음 글에서 다룰 **신뢰구간(Confidence Interval)**이 바로 이 불확실성을 정량화하는 도구다. MLE의 점근 정규성이 여기서 핵심 역할을 한다.
