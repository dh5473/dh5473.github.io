...
이 증명이 작동하려면 **분산이 유한**해야 한다. 다만 유한 분산은 체비셰프 부등식 기반 증명의 조건이고, WLLN 자체는 **기댓값만 존재**하면 성립할 수 있다(특성함수 기반 증명). 코시 분포(Cauchy Distribution)는 **기댓값 자체가 정의되지 않기 때문에** 큰 수의 법칙이 성립하지 않는다. 코시 분포에서 표본 평균을 아무리 많이 모아도 수렴하지 않는다는 사실은 꽤 반직관적이다.
...
- **유한 분산**: $\sigma^2 < \infty$여야 한다. 코시 분포처럼 기댓값 자체가 존재하지 않는 분포에서는 CLT가 성립하지 않는다.
