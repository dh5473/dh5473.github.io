...
비복원 추출로 $n$개를 뽑으면 원래 표본과 완전히 동일한 집합이 된다. 복원 추출이어야 매번 다른 구성의 표본이 만들어지고, 그로부터 통계량의 변동성을 추정할 수 있다. 평균적으로 한 번의 부트스트랩 표본에는 원래 관측값의 약 63.2%만 포함된다 ($1 - (1 - 1/n)^n \approx 1 - e^{-1}$).
...
두 상수를 손으로 계산하려면 jackknife까지 동원해야 해서 코드가 길고 실수하기 쉽다. 다행히 `scipy.stats.bootstrap`이 `method='BCa'` 옵션으로 이 계산을 모두 처리해 준다. 아래 Python 실습에서 바로 확인한다.
...
표준 정규 근사의 신뢰구간은 오차가 $O(n^{-1/2})$인 반면, BCa 방법은 $O(n^{-1})$로 수렴한다. 이를 "second-order accurate"라 하며, 같은 표본 크기에서도 실제 커버리지가 명목 수준(95%)에 더 가깝다.
...
**💡 Glivenko-Cantelli 정리가 말하는 것**

표본이 충분히 크면, 경험적 분포 함수 $\hat{F}_n$은 모든 지점에서 동시에 진짜 분포 $F$에 수렴한다. 이는 단순히 "평균이 비슷해진다"는 수준이 아니라, **분포 전체의 형태가 일치해간다**는 훨씬 강한 보장이다. 부트스트랩이 단순한 트릭이 아니라 수학적으로 정당화되는 근거가 바로 여기에 있다.
...
앞서 직접 구현한 부트스트랩 분포와 같은 결과를 몇 줄로 얻는다. `method` 파라미터만 바꾸면 세 가지 신뢰구간 방법을 모두 적용할 수 있다.

### 커버리지 시뮬레이션

부트스트랩 신뢰구간이 실제로 95% 커버리지를 달성하는지 확인해보자. 지수분포(scale=2.0, 참 중앙값 $2\ln 2$)에서 표본 50개를 뽑아 세 가지 방법으로 95% 신뢰구간을 만들고, 구간이 참값을 포함하는지 세는 실험을 500회 반복한 결과다($B = 5{,}000$).
...
위 결과는 500회 반복 기준이다. 엄밀한 커버리지 검증에는 1,000회 이상 반복하고 $B$도 10,000 이상으로 설정해야 정밀한 결과를 얻을 수 있다. 직접 재현할 때는 실행 시간이 꽤 걸린다는 점을 감안하자.
...
(1) 극값 통계량(최댓값, 최솟값) — 복원 추출의 구조적 한계로 분포 근사 불가
(2) 극소 표본($n < 10$) — 경험적 분포가 모집단을 대표하지 못함
(3) 비정칙 문제 — 모수가 공간의 경계에 있거나 수렴 속도가 $\sqrt{n}$이 아닌 경우
...
통계에서 부트스트랩은 **불확실성을 정량화**하기 위해 쓰인다 (표준오차, 신뢰구간). ML에서 부트스트랩은 **분산을 줄이기**  위해 쓰인다 (배깅). 같은 재표본 추출 기법이 목적에 따라 완전히 다른 역할을 하는 셈이다.
