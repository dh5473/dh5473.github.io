...
[이전 글](/stats/confidence-intervals/)에서 신뢰구간을 다뤘다. "모수 $\theta$가 이 구간 안에 있을 것이다"라는 구간추정은 점추정의 불확실성을 정량화하는 강력한 도구였다. 그런데 한 가지 자연스러운 질문이 떠오른다 — **만약 우리가 주장하는 특정 값 $\theta_0$이 신뢰구간 밖에 있다면, 그 주장을 기각해도 되는 걸까?**
...
여기서 핵심적인 비대칭이 있다. **$H_0$은 "참이라고 가정"하는 것이지, "참이라고 믿는" 것이 아니다.** 검찰이 피고인의 무죄를 추정하는 것과 같다 — 충분한 증거가 있을 때만 유죄(기각)를 선고한다.
...
<div style="background: #f0f4ff; border-left: 4px solid #3182f6; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>💡 검정 통계량의 핵심 성질</strong><br>검정 통계량은 반드시 <strong>"$H_0$이 참일 때의 분포"</strong>가 알려져 있어야 한다. Z-통계량은 표준정규분포, t-통계량은 t-분포, 카이제곱 통계량은 $\chi^2$-분포를 따른다. 이 귀무분포(null distribution)가 있어야 "이 데이터가 얼마나 극단적인가"를 판단할 수 있다.</div>
...
1. **$H_0$이 참이라고 가정**한다 ($\mu = \mu_0$)
2. 이 가정 하에서 검정 통계량의 분포를 계산한다 ($Z \sim N(0, 1)$)
3. 실제 데이터로 검정 통계량의 관측값 $z_{\text{obs}}$를 계산한다
4. **$z_{\text{obs}}$가 귀무분포에서 극단적인 위치에 있다면** → $H_0$ 가정과 데이터가 양립하기 어렵다 → $H_0$을 **기각**한다

"극단적"의 기준은 **유의수준(Significance Level, $\alpha$)**으로 미리 정한다. 보통 $\alpha = 0.05$를 사용하는데, 이는 "$H_0$이 참일 때 기각할 확률을 5% 이하로 유지하겠다"는 뜻이다.
...
<div style="background: #fff3f0; border-left: 4px solid #ff6b6b; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>⚠️ 주의: "기각하지 않음" ≠ "$H_0$이 참"</strong><br>$H_0$을 기각하지 않았다고 해서 $H_0$이 참이라는 뜻은 아니다. 단지 "<strong>$H_0$을 기각할 만큼 충분한 증거가 없다</strong>"는 것이다. "증거 불충분으로 무죄"이지 "결백이 증명됨"이 아닌 것과 같다.</div>
...
p-value가 작다는 것은 "$H_0$이 참인 세계에서 이런 데이터는 매우 드물다"는 뜻이다. 따라서 **p-value $\leq \alpha$이면 $H_0$을 기각**한다.
...
<div style="background: #f0f4ff; border-left: 4px solid #3182f6; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>💡 p-value를 한 문장으로</strong><br>"$H_0$이 참인 세상에서 이 데이터(또는 더 극단적인 데이터)를 관측할 확률" — 이것이 p-value의 전부다. 이 확률이 너무 작으면($\leq \alpha$), "$H_0$이 참인 세상"이라는 가정 자체를 의심하는 것이다.</div>
...
위 예제에서 0.05라는 기준은 어디서 온 것일까? **유의수준 $\alpha$**는 검정 전에 연구자가 설정하는 **1종 오류의 허용 한계**로, "$H_0$이 참인데 실수로 기각할 확률을 최대 $\alpha$로 제한하겠다"는 선언이다.
...
<div style="background: #fff3f0; border-left: 4px solid #ff6b6b; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>⚠️ 단측 검정 남용 금지</strong><br>데이터를 본 후에 방향을 정해서 단측 검정을 하는 것은 p-hacking의 한 형태다. <strong>검정 방향은 데이터를 보기 전에 결정</strong>해야 한다. A/B 테스트에서 "새 버전이 더 나을 것이다"라는 사전 가설이 명확할 때만 단측 검정을 쓴다.</div>
...
시뮬레이션 결과가 $\alpha = 0.05$ 근처에 수렴한다. 이것이 유의수준의 의미다 — **$H_0$이 참일 때 기각할 확률이 $\alpha$로 제어된다.**
...
검정력이 낮으면 실제 효과가 있어도 발견하지 못한다. 일반적으로 **Power $\geq 0.80$**을 목표로 한다 (즉, 효과가 있을 때 80% 이상의 확률로 탐지).
...
**3. 유의수준($\alpha$)**: $\alpha$를 높이면 기각역이 넓어져서 검정력이 올라가지만, 1종 오류도 함께 증가한다.
...
<div style="background: #f0f4ff; border-left: 4px solid #3182f6; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>💡 실무적 함의</strong><br>신뢰구간이 가설검정보다 더 많은 정보를 준다. 가설검정은 "기각/기각 안 함"의 이분법이지만, 신뢰구간은 <strong>효과의 방향과 크기</strong>까지 보여준다. 논문이나 리포트에서는 p-value와 함께 신뢰구간을 반드시 보고하는 것이 좋다.</div>
...
<div style="background: #fff3f0; border-left: 4px solid #ff6b6b; padding: 16px 20px; margin: 20px 0; border-radius: 4px;"><strong>⚠️ 보고 원칙</strong><br>결과를 보고할 때는 반드시 세 가지를 함께 적어야 한다:<br>1. <strong>p-value</strong> (통계적 유의성)<br>2. <strong>효과 크기</strong> (실질적 유의성)<br>3. <strong>신뢰구간</strong> (추정의 불확실성)<br><br>"p = 0.03이므로 유의하다"만으로는 불충분하다.</div>
