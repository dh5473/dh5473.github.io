...
프로덕션 DB에 접속해서 `ps -ef | grep postgres`를 쳐봅시다.
...
수십 개의 프로세스가 뜹니다. `checkpointer`, `walwriter`, `autovacuum launcher` 같은 시스템 프로세스들 사이에 `appuser 10.0.1.5(52015) SELECT` 같은 줄이 섞여 있습니다. 저 `SELECT` 줄이 방금 던진 쿼리를 처리하고 있는 backend 프로세스입니다.

PostgreSQL은 이렇게 "프로세스 덩어리"로 돌아갑니다. MySQL에서 넘어오면 제일 먼저 낯선 부분입니다. MySQL은 커넥션마다 OS thread를 쓰지만, Postgres는 **커넥션마다 OS 프로세스를 fork**합니다. 이 한 가지 선택에서 Postgres의 거의 모든 운영 특성 — 강한 fault isolation, 무거운 커넥션 비용, 그리고 PgBouncer가 사실상 필수가 되는 이유 — 이 파생됩니다.
...
<div style="background: #f8f9fa; border: 1px solid #e9ecef; padding: 20px; margin: 24px 0; border-radius: 8px;">
  ...
  <ul style="margin: 0; padding-left: 20px;">
    <li>Parser와 rewriter는 "SQL 문자열을 내부 트리로 바꾸는" 앞단 작업 — 대부분의 경우 병목이 아님</li>
