...
이 글에서는 병렬 쿼리가 언제 켜지고 왜 안 켜지는지, 파티션 pruning이 걸리는 조건과 자주 놓치는 함정, 그리고 두 메커니즘이 만났을 때 어떤 효과가 나는지 순서대로 봅니다. [지난 글에서 다룬 조인 알고리즘](/postgres/join-algorithms/)이 여기서도 이어지는데, 병렬 쿼리에도 Parallel Hash Join이 들어오고, 파티셔닝된 두 테이블을 같은 파티션 키로 나눠 대응되는 조각끼리만 조인하는 partitionwise join이라는 별도 최적화도 붙기 때문입니다.
