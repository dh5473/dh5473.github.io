...
summary: 'PostgreSQL 운영에서 가장 자주 건드리는 세 축 — 연결 관리(max_connections, PgBouncer), 메모리 파라미터(shared_buffers, work_mem, effective_cache_size), autovacuum 튜닝 — 의 의미와 관계를 정리하고, 실전 장애 시나리오 3가지로 접목합니다.'
...
`BEGIN`으로 트랜잭션을 열고 아직 `COMMIT`이나 `ROLLBACK`을 하지 않은 세션은 VACUUM에 영향을 줍니다. Repeatable Read 이상의 격리 수준에서는 트랜잭션 시작 시 잡은 snapshot이 계속 유지되므로 **xmin horizon을 직접 붙잡습니다.** 기본 격리 수준인 Read Committed에서는 문장 사이에 snapshot을 해제하지만, 열린 트랜잭션 자체가 가진 xid가 VACUUM의 tuple 제거 판단에 영향을 줍니다. [VACUUM 글](/postgres/vacuum-and-bloat/)에서 봤듯이, xmin horizon이 전진하지 않으면 autovacuum은 그 이후에 발생한 dead tuple을 회수할 수 없습니다. 어느 격리 수준이든, 트랜잭션을 열어놓은 채 30분, 1시간이 지나면 그 사이에 쌓인 dead tuple 회수가 지연됩니다.
...
이 시리즈는 [페이지와 튜플](/postgres/heap-page-tuple/)에서 시작해, [MVCC](/postgres/mvcc-visibility/)가 어떻게 동시성을 제공하는지, [인덱스](/postgres/btree-anatomy/)와 [플래너](/postgres/planner-statistics/)가 어떻게 쿼리를 최적화하는지, [VACUUM](/postgres/vacuum-and-bloat/)이 왜 필요한지, [WAL](/postgres/wal-and-checkpoint/)이 어떻게 데이터를 지키는지를 따라왔습니다.
