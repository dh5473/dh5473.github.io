...
[2편](/llm/kv-cache/)에서 KV Cache가 LLM 서빙의 희소 자원이라는 걸 봤습니다. 그런데 그 희소한 자원을, 옛 시스템들은 이런 식으로 60~80%나 낭비하고 있었습니다. 아무리 GPU를 사도 실제로 쓰는 건 20~40%뿐이었다는 뜻입니다. 이 글에서는 그 낭비가 정확히 어디서 왔는지, 그리고 vLLM의 **PagedAttention**이 운영체제의 오래된 아이디어를 빌려 이 낭비를 4% 밑으로 줄인 방법을 파헤칩니다. 시리즈에서 "vLLM은 왜 빠른가"에 처음으로 직접 답하는 글입니다.
...
> **PagedAttention은 KV Cache를 아끼는 기술이 아니라, 희소 자원을 남김없이 쓰게 해주는 기술입니다.** 2편에서 계산한 그 빠듯한 KV 예산의 거의 전부를 실제 요청에 쓸 수 있게 되고, 그만큼 배치가 커지고, 배치가 커진 만큼 처리량이 오릅니다. vLLM이 이전 시스템 대비 처리량 2~4배를 낸 근원의 상당 부분이 여기에 있습니다.
...
이 블록 공유가 바로 **prefix caching**의 씨앗입니다. 시스템 프롬프트나 긴 문서를 반복해서 쓰는 실무 상황에서 큰 절감을 만들어내는데, 이 이야기는 5편에서 자세히 다룹니다. 지금은 "블록 단위로 관리하니 공유도 자연스럽게 가능해졌다"는 정도만 기억해두면 됩니다.
