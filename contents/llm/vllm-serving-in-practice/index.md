...
숫자는 환경마다 다르지만 세 줄의 의미는 같습니다. 첫 줄이 예산에서 가중치와 활성값을 빼고 남은 KV 몫입니다. 이 메모리는 곧 KV 블록을 몇 개 확보했느냐이고, 셋째 줄의 최대 동시성은 그 블록 수를 `max_model_len` 길이 요청 하나가 잡아야 할 블록 수로 나눈 값입니다. 모든 요청이 최대 길이까지 가더라도 18개 남짓은 동시에 감당한다는 뜻입니다. 둘째 줄은 그 결과에서 나옵니다. 최대 동시성에 `max_model_len`을 곱한 것이 풀 전체가 담을 수 있는 토큰 수입니다(역산하면 이 예시는 토큰당 약 65KB인 모델입니다). 메모리를 토큰으로 환산한 뒤 동시성을 구하는 순서가 아니라 그 반대인데, 레이어마다 KV 구조가 다른 하이브리드 모델이나 sliding window가 섞인 모델에서는 블록 단위로 세는 쪽이라야 답이 맞기 때문입니다.
...
정규식, 선택지, 임의 문법이 필요하면 vLLM 확장 파라미터 `structured_outputs`(json, regex, choice, grammar)를 씁니다. thinking을 켠 모델에서는 `--reasoning-parser`를 지정해두어야 사고 과정에 마스크를 걸지 않고 최종 답변부터 형식을 강제합니다. 파서 없이 형식을 걸면 첫 토큰부터 문법이 강제되어 사고 과정 자체가 망가집니다.
...
KV Cache에도 같은 분할이 적용됩니다. KV head도 나눌 head가 남아 있는 동안에는 TP 수만큼 나뉘므로 KV Cache가 GPU마다 1/N씩 분산되고, 가중치 몫도 1/N로 줄어 있으니 GPU당 KV 여유가 이중으로 늘어납니다. 모델이 한 장에 들어가더라도 KV 풀을 키우려고 TP를 쓰는 경우가 있는 이유입니다. 다만 여러 query head가 KV head 하나를 공유하는 GQA 구조라 KV head가 몇 개 없는 모델에서, TP를 KV head 수보다 크게 잡으면, 같은 KV head를 여러 GPU가 복제해서 들고 가며 캐시 효율이 떨어집니다.
