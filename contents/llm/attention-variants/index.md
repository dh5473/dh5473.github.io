...
Transformer 블록 안에서 파라미터가 가장 많은 부분은 FFN이고 MoE는 이 FFN을 전문가로 쪼개 토큰당 활성 파라미터를 줄였습니다. 어텐션 쪽에도 비슷한 낭비가 있습니다. 모든 헤드가 자기만의 K, V를 캐시에 쌓는 구조이기 때문이죠.

디코더 LLM은 토큰을 하나씩 생성합니다. 토큰 하나를 만들 때마다 이전 토큰들의 K, V를 메모리에서 읽어야 하고 이것이 KV Cache입니다. 시퀀스가 길어질수록 캐시도 커지고 128K 컨텍스트에 KV head가 64개인 모델이라면 수십 GB에 달할 수 있습니다.

GQA는 여러 Q 헤드가 K, V 한 벌을 나눠 쓰게 해서 이 캐시를 줄였습니다. Q 헤드 64개가 KV 8벌을 공유하면 캐시가 8분의 1로 줄어듭니다. 그런데 GQA가 줄인 것은 K, V의 **벌 수**이지 각 K, V 벡터의 크기(head_dim)는 128차원 그대로입니다.

KV head를 극단까지 줄여 1개만 남겨도(MQA) 토큰당 캐시의 하한은 $2 \times d_h \times L$입니다. K와 V 각 1벌의 head_dim에 레이어 수를 곱한 것이고 벡터 자체를 줄이지 않는 한 이 하한은 넘을 수 없습니다.

2024년, DeepSeek-V2가 완전히 다른 접근을 제안했습니다. K, V를 나눠 쓰지 않고 압축하는 것, 벌 수가 아니라 **차원을 줄이는 것**입니다. 이것이 MLA(Multi-head Latent Attention)입니다.
...
MQA를 제안한 Shazeer(2019)의 논문 제목은 "Fast Transformer Decoding"입니다. 품질 논문이 아니라 속도 논문이죠. 디코더가 토큰을 하나 생성할 때 새 토큰의 Q 벡터 하나와 캐시 전체의 K, V를 사용합니다. 연산량은 Q 하나분뿐인데 캐시 전체를 메모리에서 읽어야 합니다.

A100의 연산 능력은 312 TFLOPS인데 메모리 대역폭은 2 TB/s입니다. 연산 유닛이 놀고 있어도 데이터가 도착하지 않아 기다리는 상태, 이것이 디코드 단계의 메모리 대역폭 병목입니다.

KV Cache를 줄이면 읽어야 할 데이터가 줄어 이 병목이 완화됩니다. K, V를 모든 헤드가 하나씩만 공유하면 읽는 양이 $1/h$로 줄어들고 이것이 MQA의 핵심 아이디어입니다.

대가는 표현력입니다. 같은 K, V로 $h$개의 서로 다른 질문에 답해야 하니 주어를 찾는 헤드와 목적어를 찾는 헤드가 같은 간판을 보게 됩니다. "나는 주어다"와 "나는 목적어가 아니다"를 한 간판에 다 적어야 하니 어느 질문에도 딱 맞지 않는 타협적인 표현이 되고 맙니다.

GQA(Ainslie et al., 2023)는 이 타협의 정도를 조절합니다. Q 헤드를 $g$개 그룹으로 묶고 그룹마다 K, V 한 벌을 공유하는 방식입니다. $g = 1$이면 MQA이고 $g = h$이면 MHA입니다.
...
GQA 논문의 ablation에서 GQA-8의 품질은 MHA와 거의 같았고(평균 47.1 vs 47.2) MQA(46.6)보다 0.5점 높았습니다. 속도는 MHA의 5.4배였습니다. 기존 MHA 체크포인트에서 원래 학습량의 5%만으로 변환할 수 있다는 실용적 이점까지 있었습니다. 8개 그룹이 최적점이라는 결론이었고 Llama 2 70B 이후 오픈 모델의 기본값이 됐습니다.

다만 공유에는 구조적 한계가 있습니다. KV head를 아무리 줄여도 각 K, V 벡터의 차원 $d_h$는 건드리지 못합니다. $d_h$가 128이면 MQA(KV head 1개)라도 토큰당 $2 \times 128 = 256$차원분을 레이어마다 저장해야 하고 60개 레이어, 128K 컨텍스트, fp16 기준이면 요청 하나의 KV Cache가 약 3.8GB입니다.

$d_h$ 자체를 줄이면 어떨까요? 어텐션이 구분할 수 있는 패턴의 해상도가 떨어집니다. 128차원 공간에서 가까운 두 벡터를 64차원으로 투영하면 구분이 어려워질 수 있습니다. 공유만으로는 이 바닥을 뚫을 수 없고 차원을 직접 줄이면 품질이 떨어집니다.
...
캐시에 저장하는 것은 $c_t$뿐입니다. 차원이 $d_c$로 전체 K, V보다 훨씬 작죠. 추론 시에는 $c_t$를 캐시에서 읽고 $K_t$, $V_t$를 실시간으로 복원합니다.
...
DeepSeek-V2의 구체적인 값을 보면 $d_\text{model} = 5{,}120$이고 $d_c = 512$입니다. 5,120차원을 512차원으로 10분의 1로 압축한 셈이죠. 이 512차원짜리 $c_t$ 하나로 128개 헤드의 K와 V를 모두 복원합니다. 참고로 DeepSeek-V2는 Q도 별도의 잠재 벡터로 압축하는데($d_{c'} = 1{,}536$) 이쪽은 파라미터 절감 목적이고 KV Cache와는 무관합니다.
...
MLA의 576은 GQA-8의 2,048보다 작고 MHA 대비로는 93% 이상 줄어듭니다. DeepSeek-V2 논문은 이 수치가 GQA-2.25(KV head 2.25개 분량)에 해당한다고 밝혔습니다. KV head 2.25개 수준의 캐시로 MHA보다 나은 품질을 달성한 셈입니다.
...
표에서 MLA의 캐시가 $d_c$만이 아니라 $d_c + d_h^R$인 이유가 있습니다. 위치 인코딩 때문입니다.

## 위치 정보와 압축의 충돌

MLA의 압축이 다운프로젝션과 업프로젝션만으로 끝났다면 아이디어는 간단합니다. 그런데 현재 대부분의 LLM이 쓰는 위치 인코딩 방식이 이 압축과 정면으로 충돌합니다.

RoPE(Rotary Position Embedding)는 K 벡터에 위치에 따른 회전을 적용합니다. 같은 단어라도 문장에서의 위치가 다르면 K 값이 달라지고 이 덕분에 Q와 K의 내적에 상대 위치 정보가 반영되죠. 2026년 기준으로 Llama, Qwen, Mistral, DeepSeek 등 주요 모델이 모두 RoPE를 쓰고 있으니 MLA가 RoPE와 양립해야 하는 것은 선택이 아닙니다.
...
$W^{UK}$는 위치와 무관한 고정 행렬이므로 Q 쪽으로 옮길 수 있습니다. $\tilde{q}_t = W^{UK\top} q_t$를 한 번만 계산해 두면 이후 모든 과거 토큰의 $c_s$와 직접 내적하면 됩니다. K를 복원하는 연산 자체가 사라지는 셈이죠. 이것을 흡수(absorption)라 부릅니다.
...
MLA에도 대가는 있습니다. 텐서 병렬(TP)로 여러 GPU에 나눌 때 GQA는 KV head를 GPU 수로 나눌 수 있습니다. 반면 MLA의 잠재 벡터 $c_t$는 헤드 단위가 아니므로 분할할 수 없고 모든 GPU가 전체를 들고 있어야 합니다. TP=4일 때 GQA-8의 GPU당 캐시는 $2{,}048 / 4 = 512$인데 MLA는 576 그대로입니다. GPU를 늘릴수록 GQA의 캐시는 줄어드는데 MLA는 그렇지 않죠.

DeepSeek-V3(2024)는 MLA를 V2와 동일한 차원($d_c = 512$, $d_h^R = 64$)으로 유지했습니다. 그런데 DeepSeek-V4(2026)는 MLA를 제거하고 시퀀스 축을 압축하는 방식(CSA, HCA)으로 전환했습니다. 토큰의 표현 차원이 아니라 캐시에 남기는 토큰 수 자체를 줄이는 접근입니다. MLA를 만든 팀이 2년 만에 MLA를 대체한 셈이니 캐시를 줄이는 최적의 경로는 아직 열려 있습니다.
