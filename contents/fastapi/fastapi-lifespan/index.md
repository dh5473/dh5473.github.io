...
DB 커넥션 풀이나 Redis 클라이언트는 모듈 레벨에서 한 번 생성해도 대부분 잘 동작합니다. lifespan이 실질적으로 필요한 건 ML 모델처럼 로딩에 수 초가 걸리고, 완료 전에 요청을 받으면 안 되는 리소스입니다. 이 부분은 뒤에서 별도로 다루고, 여기서는 lifespan 패턴에 익숙해지기 위한 예시로 흔한 리소스들을 살펴보겠습니다.
